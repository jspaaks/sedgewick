...
static void qunionw (const Edge, const int n, int (*)[n], int (*)[n], int *);
...
static void qunionw (const Edge edge, int n, int (*labels)[n], int (*counts)[n], int * naccess) {
    int i = edge.p;
    int j = edge.q;
    while (i != (*labels)[i]) {
        i = (*labels)[i];
