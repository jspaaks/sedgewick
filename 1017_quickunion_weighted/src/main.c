...
static void qunionw (const Edge, const int n, int (*)[n], int (*)[n]);
...
int main (void) {
    ...
    for (int iedge = 0; iedge < nedges; iedge++) {
        ...
        qunionw(edge, nlabels, &labels, &counts);
...
static void qunionw (const Edge edge, int n, int (*labels)[n], int (*counts)[n]) {
    int i = edge.p;
    int j = edge.q;
    while (i != (*labels)[i]) {
        i = (*labels)[i];
