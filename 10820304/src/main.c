...
void print_data(const int ncols, const int (*ranges)[ncols], const int nrows, const int (*nsamples)[nrows], const float (*data)[nrows][ncols], const char *);
...
void print_data(const int ncols, const int (*ranges)[ncols], const int nrows, const int (*nsamples)[nrows], const float (*data)[nrows][ncols], const char * title) {
    fprintf(stdout, "\n%s\n", title);
    fprintf(stdout, "        r");
    for (int icol = 0; icol < ncols; icol++ ) {
